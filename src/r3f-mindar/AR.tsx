...
interface ARProviderProps {
  ...
  autoplay?: boolean;
  imageTargets?: any; // Replace with the actual type if known
  maxTrack?: number;
...
const ARProvider = forwardRef<any, ARProviderProps>(
  (
    {
      children,
      autoplay,
    ...
    const controllerRef = useRef<ImageTargetController | FaceTargetController>(
      null!
    );
    ...
    const startTracking = useCallback(async () => {
      if (ready) {
        let controller;
        if (imageTargets && webcamRef.current && webcamRef.current.video) {
